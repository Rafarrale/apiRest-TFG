...
var auxTipo = null;
var numAleatorio = Math.round(Math.random() * (1000 - 1) + 1);
module.exports = {
    connectToMQTT: mqtt.connect('mqtt://192.168.2.20:1883', { clean: false, clientId: numAleatorio }),
                        ...
                        db.collection("casa").findOne(query, filtro, function (err, result) {
                            ...
                            if (result != null) {
                                ...
                                db.collection("dispositivo").findOne({ _id: new ObjectID(myId) }, function (err, obj) {
                                    if (err) throw err;
                                    if (obj != null) {
                                        client.publish(myId, '201', { qos: 2, retain: false });
                                    } else {
